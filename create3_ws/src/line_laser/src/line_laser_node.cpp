...
public:
    ScanToCloudNode() : Node("scan_to_cloud_node") {
        target_frame_ = "odom";
        ...
        cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
            "/accumulated_cloud", 10);

        RCLCPP_INFO(this->get_logger(),
                    "Scan to PointCloud2 converter node (manual) has started.");
    }
    ...
    void scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
        try {
            ...
            projector_.transformLaserScanToPointCloud(
                msg->header.frame_id, *msg, current_cloud_ros, *tf_buffer_);
            ...
            tf2::doTransform(current_cloud_ros, current_cloud_transformed_ros,
                             transform_stamped);
            ...
            if (!is_initialized_) {
                ...
                if (accumulated_cloud_.fields !=
                        current_cloud_transformed_ros.fields ||
                    accumulated_cloud_.point_step !=
                        current_cloud_transformed_ros.point_step) {
                ...
                size_t new_data_size =
                    current_cloud_transformed_ros.data.size();

                // 调整累积点云数据区的大小以容纳新数据
                accumulated_cloud_.data.resize(old_data_size + new_data_size);

                // 将新数据拷贝到累积点云数据的末尾
                std::copy(current_cloud_transformed_ros.data.begin(),
