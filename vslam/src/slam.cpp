...
int main( int argc, char** argv )
{
    ...
    FRAME curFrame = readFrame( currIndex, pd ); // 上一帧数据

    CAMERA_INTRINSIC_PARAMETERS camera = GetDefaultCamera();
    OrbComputerKeyPointsAndDesp(curFrame);
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud = image2PointCloud( curFrame.rgb, curFrame.depth, camera );
    ...
    *******************************/
    ...
    for ( currIndex=startIndex+1; currIndex<endIndex; currIndex++ )
    {
        ...
        OrbComputerKeyPointsAndDesp( currFrame ); //提取特征
